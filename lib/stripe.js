 ...
 * @param {Function} callback Signature: err, customerData
 */
...
/**
 * Get all charges for this customer (actually only the latest 100 but for this demo we won't create more than 100 charges)
 * @param {Function} callback Signature: err, charges
 */
stripe.getAllCharges = function (stripeCustomerId, callback) {
  request.get({ uri: stripeApiUrl + '/charges?count=100&customer=' + stripeCustomerId, auth: { username: stripeApiToken } }, function (err, res, data) {
    if (err) { return callback(err); }
